...
export default class Data extends React.Component {
  ...
  async _loadSummary() {
    ...
    try {
      ...
      if (!this._cancelled)
        this.setState({
          error: err?.message || "Failed to load Data module summary",
          loading: false,
        });
    }
  }
      ...
      <div>
        <div className="cafe-card">
          ...
          <InfoGrid
            items={[
              ...
              {
                label: "Shape",
                value: `${dataset?.shape?.nObs || 0} obs x ${dataset?.shape?.nVars || 0} vars`,
              },
              {
                label: "Matrix",
                value: `${dataset?.matrix?.dtype || "unknown"}${dataset?.matrix?.sparse ? " | sparse" : " | dense"}`,
              },
        ...
        <div className="cafe-card">
          ...
          <div className="cafe-data-section">
            ...
            <pre className="cafe-json-block">
              {JSON.stringify(fateAnnData.priorInformation || {}, null, 2)}
            </pre>
          </div>
        ...
        <div className="cafe-card">
          ...
          {!trajectories.length ? (
            <div className="cafe-note">
              No trajectory history is available for the current dataset.
            </div>
          ) : (
        ...
        <div className="cafe-card">
          ...
          {!colorMappings.length ? (
            <div className="cafe-note">
              No color mappings were found in the current dataset summary.
            </div>
          ) : (
                    ...
                    <div className="cafe-color-swatch-list">
                      {mapping.items.map((item) => (
                        <div
                          ...
                          className="cafe-color-swatch-item"
                        >
