...
class IntegrationPreview extends React.Component {
  render() {
      ...
      return (
        <div className="cafe-table-wrap" style={{ marginTop: "10px" }}>
          <table className="cafe-table">
            <thead><tr><th>Source</th><th>Target</th><th>Weight</th><th>Sign</th></tr></thead>
            <tbody>
              {rows.slice(0, 8).map((r) => (
                <tr key={`${r.source}-${r.target}`}><td>{r.source}</td><td>{r.target}</td><td>{formatMetricValue(r.weight)}</td><td>{r.sign}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
    ...
    return (
      <div className="cafe-table-wrap" style={{ marginTop: "10px" }}>
        <table className="cafe-table">
          <thead><tr><th>Term</th><th>Overlap</th><th>Score</th><th>Genes</th></tr></thead>
          <tbody>
            {rows.slice(0, 8).map((r) => (
              <tr key={`${item.key}-${r.term}`}><td>{r.term}</td><td>{r.overlap}</td><td>{formatMetricValue(r.score)}</td><td>{r.genes || "n/a"}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
...
class Sparkline extends React.Component {
  render() {
    ...
    return <svg className="cafe-sparkline-svg" viewBox={`0 0 ${W} ${H}`}><path d={d} /></svg>;
  }
}
...
class MultiTrendChart extends React.Component {
  render() {
    ...
    const prepared = (seriesList || []).filter((s) => s.points?.length).map((s, i) => ({
      ...s,
...
export default class Explorer extends React.Component {
  ...
  async _loadSummary() {
    ...
    try {
      ...
      this.setState((prev) => {
        ...
        if ((!prev.sortMetric || !nextMetricKeys.includes(prev.sortMetric)) && nextMetricKeys.length) {
          patch.sortMetric = nextMetricKeys[0];
        }
        return patch;
      });
    } catch (err) {
      if (!this._cancelled) this.setState({ error: err?.message || "Failed to load Explorer summary", loading: false });
    }
  }
  ...
  _visibleBenchmarkRows() {
    ...
    const filtered = query
      ? rows.filter((r) => String(r.id || "").toLowerCase().includes(query))
      : [...rows];
    ...
    filtered.sort((a, b) => {
      ...
      const na = tryNumeric(a[key]), nb = tryNumeric(b[key]);
      if (na.ok && nb.ok) return sortDirection === "asc" ? na.value - nb.value : nb.value - na.value;
      return sortDirection === "asc"
  ...
  render() {
    ...
    const { summary, loading, error, activeSubModule, sortMetric, sortDirection, searchText, geneQueryInput, selectedGenes, trendScale } = this.state;
    ...
    const handleRemoveGene = (gene) => this.setState((prev) => ({ selectedGenes: prev.selectedGenes.filter((g) => g !== gene) }));
    const handleGeneSearch = () => this.setState({ geneQuery: geneQueryInput.trim(), activeSubModule: "trends" });
        ...
        <div className="cafe-card">
          ...
          <div className="cafe-switch-row">
            {["benchmark", "comparison", "drivers", "trends", "integrations"].map((tab) => (
              <button key={tab} type="button" className={`cafe-chip-btn ${activeSubModule === tab ? "is-active" : ""}`} onClick={() => this.setState({ activeSubModule: tab })}>
              ...
              <div>
                <div className="cafe-explorer-toolbar">
                  <div className="cafe-field">
                    ...
                    <input id="explorer-search" type="text" value={searchText} onChange={(e) => this.setState({ searchText: e.target.value })} placeholder="Filter by trajectory id" />
                  </div>
                  <div className="cafe-field">
                    <label htmlFor="explorer-sort">Sort by</label>
                    <select id="explorer-sort" value={sortMetric || "id"} onChange={(e) => this.setState({ sortMetric: e.target.value })}>
                  ...
                  <div className="cafe-field">
                    ...
                    <select id="explorer-direction" value={sortDirection} onChange={(e) => this.setState({ sortDirection: e.target.value })}>
                ...
                <div className="cafe-table-wrap">
                  <table className="cafe-table">
                    <thead><tr><th>ID</th>{metricKeys.map((k) => <th key={k}>{k}</th>)}</tr></thead>
                    <tbody>
                      {visibleRows.map((row) => (
                        <tr key={row.id}><td>{row.id}</td>{metricKeys.map((k) => <td key={`${row.id}-${k}`}>{formatMetricValue(row[k])}</td>)}</tr>
                      ))}
                    </tbody>
                  </table>
                </div>
          ...
          <div>
            <div className="cafe-card">
              ...
              {!overallRanking.length ? (
                ...
                <div>
                  <div className="cafe-note" style={{ marginBottom: "8px" }}>{summary?.metricComparison?.rankingHeuristic}</div>
                  <MetricBarList items={overallRanking} />
                </div>
              )}
            </div>
            <div className="cafe-explorer-metric-grid">
              {comparisonMetrics.filter((m) => m.numeric).map((metric) => (
              ...
              {!comparisonMetrics.some((m) => m.numeric) && <div className="cafe-card"><div className="cafe-note">No numeric metric comparison is available for the current dataset.</div></div>}
            </div>
          </div>
        )}
        ...
        {activeSubModule === "drivers" && (
          <div className="cafe-card">
            ...
            {!driverGenes.available ? (
              ...
              <div>
                <div className="cafe-note" style={{ marginBottom: "8px" }}>Sources: {driverGenes.sourceKeys?.join(", ") || "n/a"}</div>
                <div className="cafe-table-wrap">
                  <table className="cafe-table">
                    <thead><tr><th>Trajectory</th><th>Rank</th><th>Gene</th><th>Score</th><th>Source</th><th>Action</th></tr></thead>
                    <tbody>
                      {(driverGenes.items || []).map((item) => (
                        <tr key={`${item.trajectoryId}-${item.rank}-${item.gene}`}>
                          <td>{item.trajectoryId}</td><td>{item.rank}</td><td>{item.gene}</td><td>{formatMetricValue(item.score)}</td><td>{item.sourceKey}</td>
                          <td><button type="button" className="cafe-chip-btn" onClick={() => handleAddGene(item.gene)}>Plot Trend</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="cafe-card">
            ...
            <div className="cafe-explorer-toolbar">
              <div className="cafe-field">
                ...
                <input id="explorer-gene-query" type="text" value={geneQueryInput} onChange={(e) => this.setState({ geneQueryInput: e.target.value })} placeholder="e.g. GATA3" />
              </div>
              <div className="cafe-form-actions">
                <button type="button" className="cafe-btn" onClick={handleGeneSearch}>Search</button>
                <button type="button" className="cafe-btn" onClick={handleClearSearch}>Clear Search</button>
              </div>
            </div>
            <div className="cafe-data-section">
              ...
              {!selectedGenes.length ? (
                ...
                <div className="cafe-key-list">
                  {selectedGenes.map((gene) => (
                    <button key={gene} type="button" className="cafe-key-pill cafe-gene-pill is-active" onClick={() => handleRemoveGene(gene)}>{gene} ×</button>
                  ))}
                </div>
              )}
            </div>
              ...
              {!geneSelection.matches?.length ? (
                ...
                <div className="cafe-key-list">
                  {geneSelection.matches.map((gene) => {
                    ...
                    return (
                      <button key={gene} type="button" className={`cafe-key-pill cafe-gene-pill ${selected ? "is-active" : ""}`} onClick={() => selected ? handleRemoveGene(gene) : handleAddGene(gene)}>
                        {selected ? `${gene} ✓` : gene}
                      </button>
                    );
                  })}
            ...
            <div className="cafe-switch-row" style={{ marginBottom: "12px" }}>
              ...
              {["raw", "normalized"].map((s) => (
                <button key={s} type="button" className={`cafe-chip-btn ${trendScale === s ? "is-active" : ""}`} onClick={() => this.setState({ trendScale: s })}>
                ...
                <div className="cafe-explorer-trend-grid">
                  {(geneTrends.series || []).map((series) => (
                    <div key={`${series.trajectoryId}-${series.gene}-${series.lineage}`} className="cafe-explorer-trend-card">
                      ...
                      <div className="cafe-note">{series.trajectoryId}{series.lineage ? ` | ${series.lineage}` : ""}{series.sourceKey ? ` | ${series.sourceKey}` : ""}</div>
                      <Sparkline points={trendScale === "normalized" ? normalizeTrendPoints(series.points || []) : series.points || []} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {integrations.map((item) => (
              <div key={item.key} className="cafe-card cafe-explorer-integration-card">
                <div className="cafe-method-header">
                  ...
                  <span className={`cafe-status-badge ${item.enabled ? "is-succeeded" : "is-unknown"}`}>{item.enabled ? "ready" : "unavailable"}</span>
                </div>
                {item.enabled && <div className="cafe-note" style={{ marginTop: "8px" }}>Items: {item.itemCount ?? "n/a"}{item.source ? ` | Source: ${item.source}` : ""}</div>}
                <div className="cafe-note" style={{ marginTop: "8px" }}>{item.message}</div>
